...
def main():
    ...
    save_dir = 'F:\\AC_data\\caiman_data_missmatch2\\'
    ...
    for n_dset in range(len(dsets_names)): #
        ...
        if not os.path.exists(save_dir + save_name + '_results_cnmf.hdf5'):
            if not missing_files_num[n_dset]:
                ...
                #%%
                if do_merge:
                    f_merge_components(images, cnm2);
                ...
                # remove very bad comps
                f_remove_components(cnm2, cnm2.estimates.idx_components_bad)
                #cnm2.estimates.select_components(use_object=True)
                print('Second bad comp removal: %d comp total' % (cnm2.estimates.C.shape[0]))
                ...
                print('After final quality est: %d comp total' % (cnm3.estimates.C.shape[0]))
                ...
                #%% Show final traces
                if plot_stuff:
                    cnm3.estimates.view_components(img=Cn)
                ...
                if save_results:
                    ...
                    cnm2.save(save_dir + save_name + '_results_cnmf_cnm2.hdf5')
                    cnm3.save(save_dir + save_name + '_results_cnmf_cnm3.hdf5')
            else:
